...
namespace Mihailik.Net
{
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
			...
			//DumpHttp.SetHelloWorldListener("http://localhost:888/");

			//string response = Encoding.ASCII.GetString(DumpHttp.ResponseGet("http://localhost:888/"));
			//response = response
			//    .Replace("\r", "[CR]\r")
			//    .Replace("\n", "[LF]\n")
			//    .Replace("[CR]\r[LF]\n", "[CR][LF]\r\n");
            ...
            //string wholeHeaderTemplate =
            //    ReaderTesting.CreateTemplate<HttpRequestHeaderReader2>("IE GET", ReaderTesting.BufferToText(Properties.Resources.ie_request_get)) +
            //    "\r\n\r\n" +
            //    ReaderTesting.CreateTemplate<HttpRequestHeaderReader2>("WebRequest GET", ReaderTesting.BufferToText(Properties.Resources.httprequest_request_get));
            ////"\r\n\r\n"+
            ////ReaderTesting.CreateTemplate<HttpRequestHeaderReader2>("WebRequest POST chunked", ReaderTesting.BufferToText(Properties.Resources.httprequest_request_post_chunked));

            //wholeHeaderTemplate.GetHashCode();


            //List<ReaderTesting.TestCase> testList = new List<ReaderTesting.TestCase>(ReaderTesting.LoadTests(File.ReadAllLines("..\\..\\Internal.StateMachine\\Tests\\QueryLine.txt")));
            //for (int i = 0; i < testList.Count; i++)
            //{
            //    ReaderTesting.TestCase test = testList[i];
            //    ReaderTesting.PerformTest<HttpRequestQueryLineReader>(test);
            //}
            //testList = new List<ReaderTesting.TestCase>(ReaderTesting.LoadTests(File.ReadAllLines("..\\..\\Internal.StateMachine\\Tests\\HeaderLine.txt")));
            //for (int i = 0; i < testList.Count; i++)
            //{
            //    ReaderTesting.TestCase test = testList[i];
            //    ReaderTesting.PerformTest<HttpRequestHeaderLineReader>(test);
            //}
            //testList = new List<ReaderTesting.TestCase>(ReaderTesting.LoadTests(File.ReadAllLines("..\\..\\Internal.StateMachine\\Tests\\WholeHeader.txt")));
            //for (int i = 0; i < testList.Count; i++)
            //{
            //    ReaderTesting.TestCase test = testList[i];
            //    ReaderTesting.PerformTest<HttpRequestHeaderReader2>(test);
            //}
            


            //string queryLineTestCaseDescription =
            //    ReaderTesting.CreateTemplate<HttpRequestQueryLineReader>(
            //        "GET 1.1",
            //        "GET / HTTP/1.1[CR][LF]\r\n"
            //        ).ToString();


            //string headerLineTestCaseDescription =
            //    ReaderTesting.CreateTemplate<HttpRequestHeaderLineReader>(
            //        "dense",
            ...
            //    ReaderTesting.CreateTemplate<HttpRequestHeaderLineReader>(
            //        "widespread-unknown",
            //        "      Any    :     Everything known  123    [CR][LF]\r\n"
            //        ).ToString()+
            ...
            //headerLineTestCaseDescription.GetHashCode();


			//TestWordReader.Test1(
			//    " ",
			//    new string[]
			//    {
			//        "AB",
			...
			//DumpHttp.ResponseGet("http://localhost:777/");


			//DumpHttp.SetHelloWorldListener("http://localhost:777/");
			//File.WriteAllBytes("..\\..\\httplistener-response-get-encoding.txt", DumpHttp.ResponseGet("http://localhost:777/"));

			//WebHeaderCollection cl = new WebHeaderCollection();
			//cl.Add("Host", "host");

			//Other();
			//SrvTest3();

			//byte[] weirdHeaders = DumpHttp.RequestBclGetWeirdHeaders("/катастрофа");

			//File.WriteAllBytes(
			//    "..\\..\\httprequest-request-get-encoding.txt",
			//    weirdHeaders);


			//Console.WriteLine("HttpRequest GET...");
			//TestMultiple(
			//    Properties.Resources.httprequest_request_get,
			//    delegate(HttpRequestReader reader, byte[] content)
			//    {
			//        Debug.Assert(!reader.IsFailed);
			//        (reader.FailureDescription+"").GetHashCode();

			//        Debug.Assert(reader.HttpMethod == "GET");
			//    });

			//Console.WriteLine("HttpRequest POST chunked...");
			//TestMultiple(
			//    Properties.Resources.httprequest_request_post_chunked,
			//    delegate(HttpRequestReader reader, byte[] content)
			//    {
			//        Debug.Assert(!reader.IsFailed);
			//        (reader.FailureDescription + "").GetHashCode();

			//        Debug.Assert(reader.HttpMethod == "POST");

			//        Debug.Assert(Encoding.ASCII.GetString(content) == "onetwothree");
			//    });

			//Console.WriteLine("IE GET...");
			//TestMultiple(
			//    Properties.Resources.ie_request_get,
			//    delegate(HttpRequestReader reader, byte[] content)
			//    {
			//        Debug.Assert(!reader.IsFailed);
			//        (reader.FailureDescription + "").GetHashCode();
