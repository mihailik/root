        ...
        (function (metadata) {
            ...
            function ensureTableTypesPopulated() {
                ...
                var tabs = [
                    ...
                    new TableType("TypeDef", 2, "The first row of the TypeDef table represents the pseudo class that acts as parent for functions and variables" + "defined at module scope." + "If a type is generic, its parameters are defined in the GenericParam table (para22.20). Entries in the" + "GenericParam table reference entries in the TypeDef table; there is no reference from the TypeDef table to the" + "GenericParam table.", metadata.TypeDef), 
                    ...
                    new TableType("MethodDef", 6, "Conceptually, every row in the MethodDef table is owned by one, and only one, row in the TypeDef table." + "The rows in the MethodDef table result from .method directives (para15). The RVA column is computed when" + "the image for the PE file is emitted and points to the COR_ILMETHOD structure for the body of the method.", metadata.MethodDef), 
                    new TableType("Param", 8, "Conceptually, every row in the Param table is owned by one, and only one, row in the MethodDef table." + "The rows in the Param table result from the parameters in a method declaration (para15.4), or from a .param" + "attribute attached to a method.", null), 
                    ...
                    new TableType("Assembly", 32, "ECMA-335 para22.2.", null), 
                    new TableType("AssemblyProcessor", 33, "ECMA-335 para22.4 Shall be ignored by the CLI.", null), 
                    new TableType("AssemblyOS", 34, "ECMA-335 para22.3 Shall be ignored by the CLI.", null), 
                    new TableType("AssemblyRef", 35, "The table is defined by the .assembly extern directive (para6.3).  Its columns are filled using directives" + "similar to those of the Assembly table except for the PublicKeyOrToken column, which is defined using the" + ".publickeytoken directive.", null), 
                    new TableType("AssemblyRefProcessor", 36, "ECMA-335 para22.7 Shall be ignored by the CLI.", null), 
                    new TableType("AssemblyRefOS", 37, "ECMA-335 para22.6 Shall be ignored by the CLI.", null), 
