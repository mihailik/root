...
{
    internal struct IgnoreCaseWordReader
    {
        enum ReaderState
        {
            KnownWord,
        ...
        public struct Generator
        {
                ...
                foreach (string word in knownWords)
                {
                    if (word.Length > startsWith.Length && word.StartsWith(startsWith, StringComparison.Ordinal))
                    {
                        ...
                        if (!nodesStartingWith.ContainsKey(nextStartsWith))
                        {
                            ...
                            byte b = (byte)word[startsWith.Length];
                            b = ToUpper(b);
                            wordNode.Byte = b;

                            if (ToUpper((byte)word[startsWith.Length]) != b)
            ...
            {
                const int caseShift = (int)'a' - (int)'A';

                if (b >= (int)'a' && b <= (int)'z')
                    return (byte)(b - caseShift);
                else
                    return b;
            }
            ...
            public IgnoreCaseWordReader GetReader()
            {
                return new IgnoreCaseWordReader(parseDefinition, stoppers, knownWords);
            }
        }
        ...
        private IgnoreCaseWordReader(byte[] parseDefinition, byte[] stoppers, string[] knownWords)
        {
            ...
            switch (currentState)
            {
                case ReaderState.KnownWord:
                    ...
                    while (true)
                    {
                        ...
                        else if (EqualIgnoreCase(nextByte, checkByte))
                        {
        ...
        static bool EqualIgnoreCase(byte nextByte, byte checkByte)
        {
            const int caseShift = (int)'a' - (int)'A';

            return
                nextByte == checkByte
                || (nextByte - caseShift == checkByte
                && nextByte >= (int)'a'
                && nextByte <= (int)'z');
        }
        ...
        public override string ToString()
        {
            ...
            else
            {
                if (currentState == ReaderState.KnownWord)
                {
                    ...
                    {
                        return "{[" + currentKnownWordIndex + "]" +
                            currentKnownWord + "... " + currentState + "}";
                    }
                    else
                    {
                        return "{[" + currentKnownWordIndex + "]" +
                            currentKnownWord.Substring(0, this.ReadByteCount) +
                            "?" + currentKnownWord.Substring(this.ReadByteCount) + " " + currentState + "}";
                    }
                }
